...
def setPositionX(realPosX, tiaCycles, fineNum, HDirection, asl):
	remainder = positionX % tiaCycles
	xor = remainder ^ tiaCycles
	AShiftLeft = xor << asl
	print('=' * 20)
	print(f'Normal\nCurrent X Position (with HBlank as offset): {realPosX} \nMoving {HDirection}\nRemainder (sbc): {remainder}\n(xor): {xor}\n(asl): {AShiftLeft}\n')
	print(f'Bin\nCurrent X Position (with HBlank as offset): {bin(realPosX)} \nMoving {HDirection}\nRemainder (sbc): {bin(remainder)}\n(xor): {bin(xor)}\n(asl): {bin(AShiftLeft)}\n')
